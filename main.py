  ...
  while True:
    ...
    if lines.isdigit():
      ...
      if  1 <= lines <= MAX_LINES:
        ...
        print("Please enter lines between 1-" + str(MAX_LINES) + ")")
    else:
...
def get_bet():
  while True:
    amount = input("Enter amount to bet on each line in $:")
    if amount.isdigit():
      amount = int(amount)
...
def spin(balance):
  ...
  while True:
    ...
    if total_bet > balance:
      print(f"You do not have enough to bet that amount, available balance: ${balance}")
    else: break
